...
class SAE(HookedRootModule):
    ...
    def __init__(
        ...
        self.cfg = cfg
        self.activation_fn = get_activation_fn(cfg.activation_fn_str, cfg.k)
        self.dtype = DTYPE_MAP[cfg.dtype]
        self.device = torch.device(cfg.device)
        self.use_error_term = use_error_term
        
        # dec orthogonal
        self.init_dec_orthogonal = cfg.init_dec_orthogonal
    ...
    def initialize_weights_basic(self):
        ...
        if self.cfg.init_dec_orthogonal:
            self.W_dec = nn.Parameter(
...
# https://github.com/jbloomAus/SAELens/blob/ad740d4e09cf748869aa806a9c31032881b06298/sae_lens/training/training_sae.py#L59
class JumpReLU(torch.autograd.Function):
    ...
    def __call__(
        ...
        return (x * (x > self.jump)).to(x)

    def setup_context(
        self, ctx: Any, inputs: tuple[torch.Tensor], output: torch.Tensor
    ) -> None:
        x = inputs
        threshold = self.jump
    ...
    def backward(self, ctx: Any, grad_output: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, None]:  # type: ignore[override]
...
# https://github.com/erichson/JumpReLU/blob/master/activationfun.py
...
def get_activation_fn(activation_fn: str, k: int = 1, postact_fn: Callable[[torch.Tensor], torch.Tensor] = nn.ReLU(), jump: float = 0.001) -> Callable[[torch.Tensor], torch.Tensor]:
